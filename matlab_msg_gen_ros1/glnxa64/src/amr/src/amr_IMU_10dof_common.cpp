...
// Common copy functions for amr/IMU_10dof
...
namespace IMU_10dof {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const amr::IMU_10dof::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  AMR_EXPORT void copy_from_arr(boost::shared_ptr<amr::IMU_10dof>& msg, const matlab::data::StructArray arr) {
    try {
        //header
  ...
  //----------------------------------------------------------------------------
  AMR_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const amr::IMU_10dof>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","gyro_val","accel_val","mag_val","temp"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // gyro_val
    outArray[0]["gyro_val"] = factory.createArray<amr::IMU_10dof::_gyro_val_type::const_iterator, double>({1, msg->gyro_val.size()}, msg->gyro_val.begin(), msg->gyro_val.end());
    // accel_val
    outArray[0]["accel_val"] = factory.createArray<amr::IMU_10dof::_accel_val_type::const_iterator, double>({1, msg->accel_val.size()}, msg->accel_val.begin(), msg->accel_val.end());
    // mag_val
    outArray[0]["mag_val"] = factory.createArray<amr::IMU_10dof::_mag_val_type::const_iterator, double>({1, msg->mag_val.size()}, msg->mag_val.begin(), msg->mag_val.end());
    // temp
