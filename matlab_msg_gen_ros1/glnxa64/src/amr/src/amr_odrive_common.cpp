...
// Common copy functions for amr/odrive
...
namespace odrive {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const amr::odrive::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  AMR_EXPORT void copy_from_arr(boost::shared_ptr<amr::odrive>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //left_vel
        const matlab::data::TypedArray<int16_t> left_vel_arr = arr[0]["left_vel"];
        msg->left_vel = left_vel_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'left_vel' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'left_vel' is wrong type; expected a int16.");
    }
    try {
        //right_vel
        const matlab::data::TypedArray<int16_t> right_vel_arr = arr[0]["right_vel"];
        msg->right_vel = right_vel_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'right_vel' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'right_vel' is wrong type; expected a int16.");
    }
  }
  //----------------------------------------------------------------------------
  AMR_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const amr::odrive>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","left_vel","right_vel"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // left_vel
    outArray[0]["left_vel"] = factory.createScalar(msg->left_vel);
    // right_vel
    outArray[0]["right_vel"] = factory.createScalar(msg->right_vel);
