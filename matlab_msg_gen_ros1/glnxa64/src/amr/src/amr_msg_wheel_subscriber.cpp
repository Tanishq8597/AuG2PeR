...
namespace wheel {
  MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const amr::wheel>& msg);
} //namespace wheel
} //namespace matlabhelper
} //namespace amr
class AMR_EXPORT amr_msg_wheel_subscriber : public MATLABSubscriberInterface {
  ...
  public:
    ...
    void callback(const amr::wheel::ConstPtr& msg){
      if(mSd){
        auto outArray = amr::matlabhelper::wheel::get_arr(mFactory, msg);
        appendAndSendToMATLAB(mSd, mSendDataToMATLABFunc, outArray);
      }
    }
    amr_msg_wheel_subscriber()
        : MATLABSubscriberInterface() {
    }
    virtual ~amr_msg_wheel_subscriber() {
    }
    virtual intptr_t createSubscription(const std::string& topic_name,
        ...
        mSub = std::make_shared<ros::Subscriber>(
                n->subscribe<amr::wheel>(
                            topic_name, 
                            buffer_size, 
                            &amr_msg_wheel_subscriber::callback, 
                            this,
...
CLASS_LOADER_REGISTER_CLASS(amr_msg_wheel_subscriber, MATLABSubscriberInterface)
#ifdef _MSC_VER
#pragma warning(pop)
#else
