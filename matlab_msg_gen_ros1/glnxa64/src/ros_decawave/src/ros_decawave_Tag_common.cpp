...
// Common copy functions for ros_decawave/Tag
...
namespace Tag {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const ros_decawave::Tag::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  ROS_DECAWAVE_EXPORT void copy_from_arr(boost::shared_ptr<ros_decawave::Tag>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //n_anchors
        const matlab::data::TypedArray<int16_t> n_anchors_arr = arr[0]["n_anchors"];
        msg->n_anchors = n_anchors_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'n_anchors' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'n_anchors' is wrong type; expected a int16.");
    }
  }
  //----------------------------------------------------------------------------
  ROS_DECAWAVE_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const ros_decawave::Tag>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","x","y","z","qf","n_anchors"});
    // header
    ...
    // n_anchors
    outArray[0]["n_anchors"] = factory.createScalar(msg->n_anchors);
